...
function readStudentsTable($sortby, $sortorder)
{
    ...
    while ($stmt->fetch()) {
        ...
        $lines .= "<td>" . '<a href="mailto:' . $email_from_db .'" class="link"><img alt="meili saatmise nupp" name="email-label" src="media/Message.png" width="15px" height="15px" hspace="10" /></a>';        $lines .= "<td>" . '<button class="delete-button" type="submit" name="delete-student" value="' . $student_id_from_db . '"> <img class="delete-button-img" src="media/delete.png" alt="kustutamise nupp"></button>' . "</td> \n";
        $lines .= "</tr> \n";
    }
    if (!empty($lines)) {
        $notice = '<table class="tags-form-table">' . "\n";
